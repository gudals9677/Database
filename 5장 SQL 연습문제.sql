...
#실습 1-20
select * from `order` where `ordercount` >= 3 order by `orderCount`
desc, `orderproduct` asc;
...
as `최고가` from `product` group by `company` order by `company`asc;
...
#실습 1-29
select `orderid`,`prodname` from `order` as a join `product` as b
