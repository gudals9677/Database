...
insert into `Customer` values ('c110','정약용','010-1234-1019','경기도 광주시','2022-01-10');
...
#실습 1-25
select `orderproduct` as `주문 상품번호`, sum(`ordercount`) as
 `총 주문수량` from `order` group by `orderproduct` order by `주문 상품번호`;

#실습 1-26
select `company` as `제조업체`, count(*) as `제품수`, max(`price`)
 as `최고가` from `product` group by `company` order by `company`;

#실습 1-27
select `company` as `제조업체`, count(*) as `제품수`, max(`price`)
 as `최고가` from `product` group by `company` having `제품수` >=2;

#실습 1-28
select `orderproduct`, `orderid`, sum(`ordercount`)
 as `총 주문수량` from `order` group by `orderproduct`,`orderid` order by `orderproduct`;

#실습 1-29
select a.orderid, b.prodname from `order`
 as a join  `product`
 as b on a.orderproduct = b.prodno where `orderid` = 'c102';
