...
update `nurses` set `nur_birth` = '1976-02-21';
...
insert into `nurses` values('N106101','공유','1965-11-21','M','106','주임','010-5101-6511','gong@bw.com');
...
insert into `Patients` values ('P105101', 'D105101', 'N105101', '원빈영', '660912-1234567', 'M', '대전', '010-2104-6609', 'one@daum.net', '배우');
...
insert into `treatments` values ('1081008','D108101', 'P108101', '오른쪽 발목뼈 골절', '2022-01-10 10:50:12');
...
select `doc_id`,
        ...
        `dep_name`
        from `doctors`
        as a join `departments`
        as b on a.dep_no = b.dep_no;
...
select
 ...
 from `patients`
 as a join `doctors`
 as b on a.pat_gen = b.doc_gen
 join `nurses` as c on
 a.pat_gen = c.nur_gender;
...
select 
    ...
    `treat_datetime`
from
`treatments` as a join
`patients` as b on
a.pat_id = b.pat_id
join `doctors` as c on
a.doc_id = c.doc_id
join `charts` as d on
a.treat_no = d.treat_no;
